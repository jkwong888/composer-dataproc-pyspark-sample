"""Example Airflow DAG that creates a Cloud Dataproc cluster, runs the SparkSQL example, 
...
output_file = os.path.join(
    models.Variable.get('gcs_bucket'), 'output-composer',
    datetime.datetime.now().strftime('%Y%m%d-%H%M%S')) + os.sep
input_file = os.path.join(
    models.Variable.get('gcs_bucket'), 'daily_show_guests.csv'
)
job_file = os.path.join(
    models.Variable.get('gcs_bucket'), 'etl_job.py'
)

# Arguments to pass to Cloud Dataproc job.
job_args = ['--input', input_file, '--output', output_file]
